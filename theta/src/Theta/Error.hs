...
prettyModuleError (containingModule, err) =
  [p|
    ...
    where
      ...
      containingVersion = Metadata.languageVersion $ Theta.metadata containingModule
      ...
      message = case err of
        ...
        UndefinedType name ->
          [p|The type ‘#{pretty name}’ is not defined.|] <> nameSuggestions name
        DuplicateTypeName name ->
          [p|
      ...
      -- additional hints/suggestions when names are missing
      nameSuggestions name = case suggestions of
        []          -> ""
        suggestions -> "\n\n" <> [p|
          Suggestions:
        ...
        where suggestions :: [Text]
              suggestions = primitiveVersion name <> otherModules name

      primitiveVersion Name.Name { Name.name } = case Map.lookup name primitiveNames of
        Just primitive ->
          let minVersion = Primitive.definedIn primitive
              message = [p|
                #{name} (primitive type): requires language-version ≥ #{pretty minVersion}
              |]
          in
          [message | containingVersion < minVersion]
        _ -> []
